...
class httpPost(threading.Thread):
    def __init__(self, host, port, tor, length):
    ...
    def _send_http_post(self, pause=10):
        ...
        for i in range(0, 9999):
            ...
            self.socks.send(p)
            if self.length < time.time() - self.start_time:
                break
...
def usage():
    ...
    print " -l|--lenght Define for how long (seconds)"

def main(argv):
    
    try:
        opts, args = getopt.getopt(argv, "hTt:r:p:l:", ["help", "tor", "target=", "threads=", "port=", "lenght="])
    ...
    for i in range(threads):
        t = httpPost(target, port, tor, lenght)
        rthreads.append(t)
        t.start()
...
if __name__ == "__main__":
	...
	print " */\n"
	main(sys.argv[1:])
	#TO DO  - PUT IN AN OTHER THREAD
	#processThread = threading.Thread(target=main, args=(sys.argv[1:],));
	#processThread.start();
