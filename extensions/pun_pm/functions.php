...
function pun_pm_unread_messages()
{
	...
	$return = $new_messages ? '<strong style=color:red>'.$lang_pun_pm['New link'].'</strong>' : (!$pun_pm_my_inbox_full ? $lang_pun_pm['New link'] : $lang_pun_pm['New link full']);
...
function pun_pm_get_user_group_id($id)
{
	global $forum_db;
...
function pun_pm_get_pubkey($username)
{
	global $forum_db;
	...
	($hook = get_hook('pun_pm_fn_get_pubkey_pre_query')) ? eval($hook) : null;
	...
	($hook = get_hook('pun_pm_fn_get_pubkey_end')) ? eval($hook) : null;

	return $pubkey;
}

function pun_pm_get_email($username)
{
	global $forum_db;
...
function pun_pm_inbox_enough_space($userid, $ratio = 1, $count = false)
{
	...
	if(pun_pm_get_user_group_id($userid)==1)
		return true;
...
function pun_pm_send_message($body, $subject, $receiver_username, &$message_id)
{
	...
	/*
	if (pun_pm_is_valid_pubkey($_POST['pubkey']))
	{
		//echo $receiver_username;
		$body2 = pun_pm_encrypt_body($body ,$receiver_username);
		if (trim($body2) =='')
			{
				$body= $body.'  Info: email oraz nazwa uzytkownika zapisana w kluczu jest rozna od emaila i nazwy z forum, wiec szyfrowanie nie powiodlo sie';
	...
	{
	$body = $body.'  HAU HAU! STAC POLICJA! MANDAT ZA BRAK SZYFROWANIA PANIE KIEROWCO!';	
	}
	*/
...
function pun_pm_preview($receiver, $subject, $body, &$errors)
{
	...
	if ($forum_config['p_message_bbcode'] == '1' || $forum_config['o_make_links'] == '1')
	{
		...
		if (!defined('FORUM_PARSER_LOADED'))
			require FORUM_ROOT.'include/parser.php';
		$body = preparse_bbcode($body, $errors);
		$body = pun_pm_encrypt_body($body ,$receiver);
	}
...
function pun_pm_encrypt_body($body ,$username)
{
	global $gpgtempfileurl;
	$pubkey = pun_pm_get_pubkey($username);
	$email = trim(pun_pm_get_email($username));
	
	
	$encrypted_file_url = $gpgtempfileurl.'.asc';
	if ($pubkey!='' and trim($body)!='' and $email!='')
	{
		$gpg = '/usr/bin/gpg';
		putenv("GNUPGHOME=/home/testgpguser/.gnupg");
		file_put_contents($gpgtempfileurl,$pubkey);
		//file_put_contents($pub_gpgtempfileurl,$pubkey);
		shell_exec("$gpg --trust-model always --yes --import $gpgtempfileurl");
		file_put_contents($gpgtempfileurl,$body);
		file_put_contents($encrypted_file_url,'');
		shell_exec("$gpg --trust-model always --yes -q -e -a -r $email $gpgtempfileurl");
		$body = file_get_contents($encrypted_file_url);
		if ($body=='')
		{
			shell_exec("$gpg --trust-model always --yes -q -e -a -r $username $gpgtempfileurl");
			$body = file_get_contents($encrypted_file_url);
			//file_put_contents($gpgtempfileurl.'.r0','sprobowalem');
		}
		file_put_contents($gpgtempfileurl,'');
...
function pun_pm_send_form($username = '', $subject = '', $body = '', $message_id = false, $reply_form = false, $notice = false, $preview = false )
{
	...
	if ($username!='' )
	{
		...
		if (!pun_pm_is_valid_pubkey($pubkey))
		{
			$pubkey=' ';
		}
		//$pubkey = ' AUTOMATYCZNE SZYFROWANIE CHWILOWO ZOSTALO WYLACZONE , przepraszam  - Mbank';
...
<?php ($hook = get_hook('pun_pm_fn_send_form_pubkey_output')) ? eval($hook) : null; ?>
				<div class="txt-set set<?php echo ++$forum_page['item_count'] ?>">
					<div class="txt-box textarea required">
						<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span><?php echo $lang_pun_pm['Publickey'] ?></span></label>
						<div class="txt-input"><span class="fld-input"><textarea id="fld<?php echo $forum_page['fld_count'] ?>" name="pubkey" rows="3" cols="95" required><?php echo $pubkey; ?></textarea></span></div>
					</div>
				</div>
