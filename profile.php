...
else
{
	...
	if ($section == 'about')
	{
		...
		if ($user['pubkey']!='')
			$forum_page['user_contact']['forum-pubkey'] = '<li><span>RSA public key: <textarea cols="70" rows ="10" readonly wrap="soft" syle="font-size: 4pt; font-family:Monospace">'.htmlspecialchars(validate_pubkey($user['pubkey'])).'</textarea></span></li>';
		if ($user['btcaddress']!='')
			$forum_page['user_contact']['forum-btcaddress'] = '<li><span>BTC address: '.htmlspecialchars($user['btcaddress']).'</span></li>';
		if ($user['invitedBy']!='')
			$forum_page['user_contact']['forum-invitedBy'] = '<li><span>Invited by: '.$user['invitedBy'].'</span></li>';
